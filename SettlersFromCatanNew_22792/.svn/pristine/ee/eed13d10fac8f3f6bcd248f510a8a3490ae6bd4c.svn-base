...
public final class CardController {
	...
	public void defineKnightCardActions(Button cardImage) {
		cardImage.setOnMouseClicked(e -> {
			Player owner = GameStart.siedlerVonCatan.findPlayerByID(GameStart.network.getConnectionHandler().getPlayerId());
			if(!owner.getStatus().equals("Handeln oder Bauen") && !owner.getStatus().equals("Wuerfeln")){
				GameStart.gameView.displayLargeErrorMessage(
						"You can not use a knight card right now!");
				return;
			}
			// Check if we can use the card (if we did not buy it this round)
			if (!owner.checkIfWeDidNotBuyCardThisRound(EvolutionType.KNIGHT)) {
				GameStart.gameView
	...
	public void defineRoadBuildingCardActions(Button cardImage) {
		cardImage.setOnMouseClicked(e -> {
			...
			if(!owner.getStatus().equals("Handeln oder Bauen") && !owner.getStatus().equals("Wuerfeln")){
				GameStart.gameView.displayLargeErrorMessage(
						"You can not use a road building card right now!");
				return;
			}
			// Check if we can use the card (if we did not buy it this round)
			if (!owner.checkIfWeDidNotBuyCardThisRound(EvolutionType.ROAD_BUILDING)) {
				GameStart.gameView.displayLargeErrorMessage(
			...
			GameStart.mainLogger.getLOGGER().fine("road building card played: true");
		});
	}
	...
	public void defineMonopolyCardActions(Button cardImage) {
		cardImage.setOnMouseClicked(e -> {
			...
			if(!owner.getStatus().equals("Handeln oder Bauen") && !owner.getStatus().equals("Wuerfeln")){
				GameStart.gameView.displayLargeErrorMessage(
						"You can not use a monopoly card right now!");
				return;
			}
			// Check if we can use the card (if we did not buy it this round)
			if (!owner.checkIfWeDidNotBuyCardThisRound(EvolutionType.MONOPOLY)) {
				GameStart.gameView
			...
			GameStart.mainLogger.getLOGGER().fine("monopoly card played: true");
		});
	}
	...
	public void defineYearOfPlentyCardActions(Button cardImage) {
		cardImage.setOnMouseClicked(e -> {
			...
			if(!owner.getStatus().equals("Handeln oder Bauen") && !owner.getStatus().equals("Wuerfeln")){
				GameStart.gameView.displayLargeErrorMessage(
						"You can not use a year of plenty card right now!");
				return;
			}
			// Check if we can use the card (if we did not buy it this round)
			if (!owner.checkIfWeDidNotBuyCardThisRound(EvolutionType.YEAR_OF_PLENTY)) {
				GameStart.gameView.displayLargeErrorMessage(
