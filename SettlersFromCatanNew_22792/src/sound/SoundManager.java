...
public class SoundManager {
	...
	private double musicSliderValue=10;
        ...
        try {
            ...
            backgroundMusic.addLineListener(event -> {
                if (event.getType() == LineEvent.Type.STOP) {
                    event.getLine().close();
                    GameStart.mainLogger.getLOGGER().fine("Clip finished. Playing new clip....");
                    playSoundBackGroundMusic();
                    gainControl.setValue(musicVolumeValue);
                }
            });
        } catch (Exception e) {
            GameStart.mainLogger.getLOGGER().fine("Music error: " + "BackgroundMusic" + nextMusic + ".wav");
            e.printStackTrace();
        }
    }
    ...
    public void playVictorySound() {
    	URL path = ResourcePointer.class.getResource("Triumph.wav");
        AudioClip sound = new AudioClip(path.toString());
        sound.play(soundEffectsVolumeValue);
    }
    ...
    public void playDefeatSound() {
    	URL path = ResourcePointer.class.getResource("DefeatSound.wav");
        AudioClip sound = new AudioClip(path.toString());
        sound.play(soundEffectsVolumeValue);
    }
    ...
    public void playEntranceSound() {
    	URL path = ResourcePointer.class.getResource("EntranceMainMenu.wav");
        AudioClip sound = new AudioClip(path.toString());
        sound.play(soundEffectsVolumeValue);
    }
	...
	public void changeSoundEffectsVolume(double newValue) {
		GameStart.mainLogger.getLOGGER().fine(Double.toString(newValue));

		if (newValue==10) soundEffectsVolumeValue=(float) 1.0;
		else soundEffectsVolumeValue=(float) (newValue/10-0.1);
	}
    ...
    /**
     * returns the last value received from the music slider controller (range 1-10) 
     * @return  slider representation of the value
     */
    public double getMusicVolumeValue() {
		return musicSliderValue;
	}
    /**
     * returns the last saved sound effects volume value 
     * @return float representing the volume value
     */
    public float getSoundEffectsVolumeValue() {
