...
public final class ServerMemoryLogic {
				...
				for (int j = 0; j < serverMap.getBuildings().length; j++) {
					if (!serverMap.getBuildings()[j].getType().equals("Strasse")) {
						...
						if (buildingposition.contains(serverMap.getFields()[i].getLocation())) {
							...
							//GameStart.mainLogger.getLOGGER()().fine("++++++++++" + buildingposition + " - " + serverMap.getFields()[i].getLocation());
	...
	public static void reduceAvailableResourceCardsNumber(Resources resourcesToSubstract) {
		...
		//GameStart.mainLogger.getLOGGER()().fine("resources cards available: " + availableResourceCards[0] + ", " + availableResourceCards[1]	+ ", " + availableResourceCards[2] + ", " + availableResourceCards[3] + ", 	+ availableResourceCards[4]);
	}
...
//
		if(random < availableDevelopmentCards[0]){
			availableDevelopmentCards[0]--;
			return new DevelopmentCards( 1, null, null, null, null, null);
		}
		else if(random < availableDevelopmentCards[0]+availableDevelopmentCards[1]){
			availableDevelopmentCards[1]--;
			return new DevelopmentCards(null, 1, null, null, null, null);
		}
		else if(random < availableDevelopmentCards[0]+availableDevelopmentCards[1]+availableDevelopmentCards[2]){
			availableDevelopmentCards[2]--;
			return new DevelopmentCards(null, null, 1, null, null, null);
		}
		else if(random < availableDevelopmentCards[0]+availableDevelopmentCards[1]+availableDevelopmentCards[2]
				+availableDevelopmentCards[3]){
			availableDevelopmentCards[3]--;
			return new DevelopmentCards(null, null, null, 1, null, null);
		}
		else if(random < availableDevelopmentCards[0]+availableDevelopmentCards[1]+availableDevelopmentCards[2]
				+availableDevelopmentCards[3]+availableDevelopmentCards[4]){
			availableDevelopmentCards[4]--;
...
//		}
	...
	public synchronized static PlayerForProtocol getPlayerWithLongestRoute() {
		...
		//GameStart.mainLogger.getLOGGER()().fine("<<<<Longest RODE:" + (longestRoad == 4 ? "None" : longestRoad) + ">>>>>>");
	...
	private static int getPlayerWithLongestRodeRekursive(Building building,
		...
		for (Building building1 : buildings) {
			...
			if (!building1.getType().equals("Strasse") && building.getOwner() != building1.getOwner())
				breakLoop = true;
		}
		if (!breakLoop) {
			for (Building building1 : buildings) {
